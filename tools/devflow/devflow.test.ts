...
const run = (args, input) => execFileSync(process.execPath, [cli, ...args], { cwd: root, input, encoding: "utf8" });
test.after(() => { fs.writeFileSync(state, initial); fs.writeFileSync(stage, initialStage); });
test("status exposes Stage 1 and five proofs", () => { const out = JSON.parse(run(["status", "--json"])); assert.equal(out.active_stage.id, "PVX-STAGE-1"); assert.equal(out.proof_obligations.length, 5); assert.equal(out.exit_gate.complete, false); });
