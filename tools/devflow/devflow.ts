...
function fail(message) { console.error(`devflow: ${message}`); process.exitCode = 1; }
function readJson(file) { return JSON.parse(fs.readFileSync(file, "utf8")); }
function writeJson(file, value) { fs.writeFileSync(file, `${JSON.stringify(value, null, 2)}\n`); }
function load() {
  const stage = readJson(STAGE_FILE); const state = readJson(STATE_FILE); validateStage(stage); validateState(stage, state); return { stage, state };
}
function validateStage(s) {
  ...
  const ids = s.proof_obligations.map((p) => p.id); const expected = ["PVX-S1-P1", "PVX-S1-P2", "PVX-S1-P3", "PVX-S1-P4", "PVX-S1-P5"];
...
function validateState(stage, state) {
...
function projection(stage, state) {
  ...
  return { design_baseline: stage.design_baseline, active_stage: stage.stage, stage_status: stage.stage.status, proof_obligations: proofs,
    ...
    next_eligible_action: exit ? "MANDATORY_PRE_EXIT_REVIEW" : `WORK_ON_${current.id}` };
}
